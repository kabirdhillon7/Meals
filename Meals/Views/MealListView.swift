...
struct MealListView: View {
    ...
    var body: some View {
        NavigationStack {
            List(viewModel.meals.sorted(by: { $0.name < $1.name }), id: \.id) { meal in
                NavigationLink {
                    MealDetailView(mealId: meal.id)
                } label: {
                    HStack {
                        AsyncImage(url: URL(string: meal.thumbnailString)) { phase in
                            switch phase {
                            case .empty:
                                ProgressView()
    ...
    final class ViewModel {
        ...
        func getMealsList() async {
            do {
                ...
                print("MealListViewModel - error getting meals list: \(error.localizedDescription)")
            }
        }
    }
}
